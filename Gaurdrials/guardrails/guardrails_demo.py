...
class Runner:
    @staticmethod
    async def run(agent: Agent, input: str, context: dict = None):
        context = context or {}
        
        # Run input guardrails first
        for guardrail in agent.input_guardrails:
            result = await guardrail(context, agent, input)
            if result.tripwire_triggered:
                raise InputGuardrailTripwireTriggered(result.output_info)
        
        # Simulate agent processing
        response = f"Processed by {agent.name}: {input}"
...
@input_guardrail
async def input_math_check(ctx, agent, input):
    ...
    result = await Runner.run(check_input_agent, input)
